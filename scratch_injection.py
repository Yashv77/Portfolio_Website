...
SUPABASE_URL = 'https://ehrnakvepsutuobmtzcf.supabase.co'
SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVocm5ha3ZlcHN1dHVvYm10emNmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUxNjI4MDgsImV4cCI6MjA5MDczODgwOH0.X8SpZvlC0aoE5GhUwl39YgwOgPx2RiT5YfNrxN8Woug'
HEADERS = {
    'apikey': SUPABASE_KEY,
...
response = requests.get(f"{SUPABASE_URL}/rest/v1/portfolio_data?id=eq.1", headers=HEADERS)
data = response.json()
...
existing_projects = content.get("personalProjectsData", [])
existing_titles = [p["title"]["professional"] for p in existing_projects]

final = []
for p in new_projects:
    if p["title"]["professional"] not in existing_titles:
        final.append(p)

content["personalProjectsData"] = final + existing_projects

patch_response = requests.patch(f"{SUPABASE_URL}/rest/v1/portfolio_data?id=eq.1", headers=HEADERS, json={"content": content})
print("Push status:", patch_response.status_code)
