...
- **Display Scale Clipping Fix**: Removed `lg:justify-center` from the left sticky header in `Portfolio.jsx` and added `lg:pt-24` along with `overflow-y-auto`. Previously, if a user had a high display scaling (e.g., 150% on Windows), the logical screen height shrank significantly. The `justify-center` rule forcefully centered the ~670px tall static content within a ~500px viewport, permanently pushing the top "Yash Vardhan" heading off-screen with no way to scroll. It now aligns safely to the top and allows internal scrolling if the viewport height is extreme.
